...
import jd.core.classfile.analyzer.variable.DefaultVariableNameGenerator;
...
import jd.core.classfile.visitor.AddCheckCastVisitor;
...
import jd.core.instruction.bytecode.instruction.InvokeInstruction;
...
import jd.core.instruction.bytecode.visitor.SearchInstructionByOffsetVisitor;
...
{	
			...
			List<Instruction> list, List<Instruction> listForAnalyze)
	{
		...
		if (localVariables == null)
		{
			...
			if ((method.access_flags & Constants.ACC_STATIC) == 0)
			{
				int nameIndex = constants.addConstantUtf8(
					Constants.THIS_LOCAL_VARIABLE_NAME);
				int signatureIndex = 
				...
				((classFile.access_flags & Constants.ACC_STATIC) == 0))
			{
				// Add outer this
				int nameIndex = constants.addConstantUtf8(
					Constants.OUTER_THIS_LOCAL_VARIABLE_NAME);
				String internalClassName = classFile.getInternalClassName();
				int lastInnerClassSeparatorIndex = 
					internalClassName.lastIndexOf(Constants.INTERNAL_INNER_SEPARATOR);
			...
			AnalyzeMethodParameter(
				classFile, method, constants, localVariables, 
		...
		else
		{
			...
			int indexOfFirstLocalVariable = 
				(((method.access_flags & Constants.ACC_STATIC) == 0) ? 1 : 0) +				
				SignatureAnalyzer.GetParameterSignatureCount(methodSignature);		
		...
		{
			AnalyzeMethodCode(
				method, constants, localVariables, list, listForAnalyze);
	...
	private static void AnalyzeMethodParameter(
			ClassFile classFile, Method method, ConstantPool constants, 
			...
			DefaultVariableNameGenerator variableNameGenerator, int codeLength)
	{
		...
		List<String> parameterTypes = 
			SignatureAnalyzer.GetParameterSignatures(methodSignature);
		...
		if (parameterTypes != null)
		{
			...
			{
				final String signature = parameterTypes.get(i);

				if ((i >= firstVisibleParameterCounter) &&
					(localVariables.getLocalVariableWithIndexAndOffset(variableIndex, 0) == null))
				{
					boolean appearsOnce = SignatureAppearsOnceInParameters(
					...
					if (astore.valueref.opcode == ByteCodeConstants.EXCEPTIONLOAD)
					{
						...
						if (el.exceptionNameIndex != 0)
						{
							...
							{					
								...
								int nextOffset = 
									ByteCodeUtil.InstructionOffset(code, astore.offset);					
		...
		byte[] code, LocalVariables localVariables, IndexInstruction ii)
	{
		...
		if (lv == null)
		{
			...
			// l'offset suivant car les compilateurs place 'start_pc'
			// une instruction plus apres.
			int nextOffset = ByteCodeUtil.InstructionOffset(code, ii.offset);					
...
//	}
	...
	private static void AnalyzeMethodCode(
			Method method, ConstantPool constants, 
			...
			List<Instruction> listForAnalyze)
	{
				...
				case ByteCodeConstants.PUTSTATIC:
					{
						...
						if (ps.valueref.opcode == ByteCodeConstants.ILOAD)
						{
							// Contrainte du type de la variable liée à ILoad par
				...
				case ByteCodeConstants.PUTFIELD:
					{
						...
						if (pf.valueref.opcode == ByteCodeConstants.ILOAD)
						{
		...
		for (int i=0; i<length; i++)
		{
			...
			case NUMBER_TYPE:
				lv.signature_index = constants.addConstantUtf8(
						GetSignatureFromTypesBitField(lv.typesBitField));
				break;
			case OBJECT_TYPE:
				...
				AddCastInstruction(method, constants, list, localVariables, lv);
				break;
			}
		}
	}
			...
			Instruction instruction)
	{
		...
		if (lv == null)
		{
			...
			{
				typesBitField = CreateTypesBitField(signature);				
			}
		...
		else
		{
			...
			else
			{
				...
				int typesBitField = CreateTypesBitField(signature);
				...
				default:
					...
					int typesBitFieldLV = CreateTypesBitField(signatureLV);
			...
			Instruction instruction, int varIndex)
	{
		...
		for (int j=0; j<nbrOfArgs; j++)
		{
			...
			{
			case ByteCodeConstants.ILOAD:
				LoadInstruction li = (LoadInstruction)arg;
				if (li.index == varIndex)
				{
					...
					if (lv != null)
						lv.typesBitField &= 
							CreateArgBitFields(argSignatures.get(j));
				}
				break;
			case ByteCodeConstants.ALOAD:
				li = (LoadInstruction)arg;
				if (li.index == varIndex)
				{
					LocalVariable lv = 
					...
					if (lv != null)
					{
						...
						case UNDEFINED_TYPE:
							lv.signature_index = 
								constants.addConstantUtf8(argSignatures.get(j));
							break;
						case NUMBER_TYPE:
		...
		if (lv1 != null)
		{
			...
			if (lv1.signature_index == NUMBER_TYPE)
			{
				...
				else
				{
					...
					if (SignatureAnalyzer.IsIntegerSignature(signature))
					{
						int type = CreateTypesBitField(signature);					
						if (type != 0)
			...
			Instruction instruction)
	{
		...
		else if (isExceptionOrReturnAddress == false)
		{
			...
			else
			{
				...
				if (SignatureAnalyzer.IsPrimitiveSignature(signatureLV))
				{
					// Creation de variables
					localVariables.add(new LocalVariable(
							offset, 1, -1, signatureInstructionIndex, index));
			...
			List<Instruction> listForAnalyze)
	{
		...
		for (int i=0; i<length; i++)
		{
			...
			case ByteCodeConstants.INVOKENEW:
				...
				break;
			case ByteCodeConstants.ISTORE:
				SetConstantTypesIStore(
					constants, localVariables, (IStore)instruction);	
				break;
			case ByteCodeConstants.PUTFIELD:
				{
			...
			case ByteCodeConstants.XRETURN:
				{
					if (returnedSignature == null)
					    returnedSignature = GetReturnedSignature(classFile, method);
			...
			IStore istore)
	{
		...
		case ByteCodeConstants.SIPUSH:
			final LocalVariable lv = 
				localVariables.getLocalVariableWithIndexAndOffset(
						istore.index, istore.offset);
			String signature = constants.getConstantUtf8(lv.signature_index);			
			((IConst)istore.valueref).setReturnedSignature(signature);
			break;
		}		
	}
			...
			ClassFile classFile, Method method)
	{
	    ...
	    return SignatureAnalyzer.GetMethodReturnedSignature(signature);
	}
		...
		for (int index=0; index<length; index++)
		{
			...
			if (i.opcode == ByteCodeConstants.ASTORE)
			{
				...
				if (as.valueref.opcode == ByteCodeConstants.EXCEPTIONLOAD)
				{
					...
					if (el.index == Constants.INVALID_INDEX)
						el.index = as.index;
				}
			}
		}
			...
			if (i.opcode == ByteCodeConstants.EXCEPTIONLOAD)
			{
					...
					(el.exceptionNameIndex > 0))
				{
					...
					LocalVariable localVariable = new LocalVariable(
						el.offset, 1, Constants.INVALID_INDEX, 
		...
		IndexInstruction ii, LoadInstruction load)
	{
		...
		int typesBitField = CreateArgBitFields(descriptor);
	...
	private static void AddCastInstruction(
			Method method, ConstantPool constants, List<Instruction> list, 
			LocalVariables localVariables, LocalVariable lv)
	{
		lv.signature_index = 
			constants.addConstantUtf8(Constants.INTERNAL_OBJECT_SIGNATURE);

		// Add cast instruction before all 'ALoad' instruction for local variable
		// le used type is not 'Object'.		
		AddCheckCastVisitor visitor = 
			new AddCheckCastVisitor(constants, localVariables, lv);
		...
		// Add cast instruction in return instruction
		Instruction lastInstruction = list.get(length - 1);
		
		if (lastInstruction.opcode == ByteCodeConstants.XRETURN)
		{
			AttributeSignature as = method.getAttributeSignature();
		    int signatureIndex = (as == null) ? 
		    		method.descriptor_index : as.signature_index;
		    ...
		    if (! SignatureAnalyzer.IsPrimitiveSignature(methodReturnedSignature) &&
				! Constants.INTERNAL_OBJECT_SIGNATURE.equals(methodReturnedSignature))
		    {
		    	methodReturnedSignature = 
		    		SignatureAnalyzer.GetInnerName(methodReturnedSignature);
			    int returnedSignatureIndex = 
			    	constants.addConstantUtf8(methodReturnedSignature);
			    int classIndex = 
			    	constants.addConstantClass(returnedSignatureIndex);		    	
			    ReturnInstruction ri = (ReturnInstruction)lastInstruction;
			    
			    ri.valueref = new CheckCast(
					ByteCodeConstants.CHECKCAST, ri.valueref.offset, 
					Instruction.UNKNOWN_LINE_NUMBER, classIndex, ri.valueref);
		    }
		}
	}

	private static int CreateTypesBitField(String signature)
	{
		/*
	...
	private static int CreateArgBitFields(String signature)
	{
		/*
	...
	private static String GetSignatureFromTypesBitField(int typesBitField)
	{
		/* 
