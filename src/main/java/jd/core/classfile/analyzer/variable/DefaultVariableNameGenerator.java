package jd.core.classfile.analyzer.variable;
...
public class DefaultVariableNameGenerator implements VariableNameGenerator
{
			...
			String signature, boolean appearsOnce)
	{
		...
		String name = (index > 0) ? "paramArrayOf" : "param";
		return generateValidName(
				name + GetSuffixFromSignature(signature.substring(index)), 
				appearsOnce);			
	}
	...
	private static String FormatSignature(String signature)
	{
		...
		int index = signature.indexOf(Constants.INTERNAL_BEGIN_TEMPLATE);
		if (index != -1)
			signature = signature.substring(0, index);

		index = signature.lastIndexOf(Constants.INTERNAL_INNER_SEPARATOR);
		if (index != -1)
			signature = signature.substring(index+1);
		
		index = signature.lastIndexOf(Constants.INTERNAL_PACKAGE_SEPARATOR);
		if (index != -1)
			signature = signature.substring(index+1);
		
		signature = 
			Character.toUpperCase(signature.charAt(0)) + signature.substring(1);
	...
	private String generateValidName(String name, boolean appearsOnce)
	{
