...
{	
	...
	public void visit(Instruction instruction)
	{
		...
		case ByteCodeConstants.INVOKENEW:
			replaceInArgs(((InvokeNew)instruction).args);
			break;
		...
		case ByteCodeConstants.INVOKESTATIC:
			replaceInArgs(((Invokestatic)instruction).args);
			break;
	...
	private Instruction match(Instruction i)
	{
		if (i.opcode == ByteCodeConstants.INVOKEVIRTUAL)
		{
			...
			if ((cmr.class_index == constants.stringBufferClassIndex) ||
				(cmr.class_index == constants.stringBuilderClassIndex))
			{
	...
	private Instruction match(Instruction i, int classIndex)
	{
		if (i.opcode == ByteCodeConstants.INVOKEVIRTUAL)
		{
			...
			ConstantMethodref cmr = 
				constants.getConstantMethodref(insi.index);
			...
			if (cmr.class_index == classIndex)
			{
				...
				if ((cnat.name_index == constants.appendIndex) && 
					(insi.args.size() == 1))
				{
					...
					else
					{
						return new BinaryOperatorInstruction(
							...
							4,  Constants.INTERNAL_STRING_SIGNATURE, "+",
		...
		else if (i.opcode == ByteCodeConstants.INVOKENEW)
		{
			...
			if ((in.classIndex == classIndex) && (in.args.size() == 1))
			{
				...
				if (arg0.opcode == ByteCodeConstants.INVOKESTATIC)
				{
					...
					if (cmr.class_index == constants.stringClassIndex)
					{
						ConstantNameAndType cnat = 
							constants.getConstantNameAndType(cmr.name_and_type_index);
				
						if ((cnat.name_index == constants.valueOfIndex) && 
						    (is.args.size() == 1))
