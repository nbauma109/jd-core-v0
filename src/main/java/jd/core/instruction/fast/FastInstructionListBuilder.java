package jd.core.instruction.fast;
...
import jd.core.exception.UnexpectedElementException;
...
import jd.core.instruction.bytecode.instruction.AStore;
import jd.core.instruction.bytecode.instruction.AThrow;
...
import jd.core.instruction.bytecode.instruction.CheckCast;
import jd.core.instruction.bytecode.instruction.ConditionalBranchInstruction;
...
import jd.core.instruction.bytecode.instruction.IStore;
import jd.core.instruction.bytecode.instruction.IfCmp;
import jd.core.instruction.bytecode.instruction.IfInstruction;
import jd.core.instruction.bytecode.instruction.IndexInstruction;
...
import jd.core.instruction.bytecode.instruction.LookupSwitch;
import jd.core.instruction.bytecode.instruction.MonitorEnter;
import jd.core.instruction.bytecode.instruction.MonitorExit;
...
import jd.core.instruction.bytecode.instruction.Switch;
import jd.core.instruction.bytecode.instruction.TableSwitch;
import jd.core.instruction.bytecode.reconstructor.AssertInstructionReconstructor;
import jd.core.instruction.bytecode.reconstructor.AssignmentOperatorReconstructor;
import jd.core.instruction.bytecode.reconstructor.IfGotoToIfReconstructor;
import jd.core.instruction.bytecode.reconstructor.InitArrayInstructionReconstructor;
import jd.core.instruction.bytecode.reconstructor.RemoveDupConstantsAttributes;
import jd.core.instruction.bytecode.reconstructor.TernaryOpInReturnReconstructor;
import jd.core.instruction.bytecode.reconstructor.TernaryOpReconstructor;
import jd.core.instruction.fast.analyzer.FastCodeExceptionAnalyzer;
import jd.core.instruction.fast.analyzer.SingleDupLoadAnalyzer;
import jd.core.instruction.fast.analyzer.StoreReturnAnalyzer;
import jd.core.instruction.fast.analyzer.UnexpectedInstructionException;
import jd.core.instruction.fast.analyzer.FastCodeExceptionAnalyzer.FastCodeException;
import jd.core.instruction.fast.analyzer.FastCodeExceptionAnalyzer.FastCodeExceptionCatch;
import jd.core.instruction.fast.instruction.FastDeclaration;
import jd.core.instruction.fast.instruction.FastFor;
import jd.core.instruction.fast.instruction.FastForEach;
import jd.core.instruction.fast.instruction.FastInstruction;
import jd.core.instruction.fast.instruction.FastLabel;
import jd.core.instruction.fast.instruction.FastList;
import jd.core.instruction.fast.instruction.FastSwitch;
import jd.core.instruction.fast.instruction.FastSynchronized;
import jd.core.instruction.fast.instruction.FastTest2Lists;
import jd.core.instruction.fast.instruction.FastTestList;
import jd.core.instruction.fast.instruction.FastTry;
import jd.core.instruction.fast.instruction.FastSwitch.Pair;
import jd.core.instruction.fast.instruction.FastTry.FastCatch;
import jd.core.instruction.fast.reconstructor.DotClass118BReconstructor;
import jd.core.instruction.fast.reconstructor.DotClassEclipseReconstructor;
import jd.core.instruction.fast.reconstructor.EmptySynchronizedBlockReconstructor;
import jd.core.instruction.fast.visitor.SearchInstructionByOpcodeVisitor;
import jd.core.instruction.util.InstructionUtil;
...
public class FastInstructionListBuilder
{
		...
		throws Exception
	{
		...
		List<FastCodeException> lfce = 
			FastCodeExceptionAnalyzer.AggregateCodeExceptions(method, list);
			...
			for (int i=lfce.size()-1; i>=0; --i)
			{
				...
				if (fce.synchronizedFlag)
					CreateSynchronizedBlock(
						referenceMap, classFile, list, localVariables, fce);
				else
					CreateFastTry(
						referenceMap, classFile, method, list, localVariables, 
						fce, returnOffset);
			}

		ExecuteReconstructors(
			referenceMap, classFile, list, localVariables);
		
		AnalyzeList(
    		classFile, method, list, localVariables, offsetLabelSet, 
    		-1, -1, -1, -1, -1, returnOffset);
	...
	private static void InitDelcarationFlags(LocalVariables localVariables)
	{
		...
		int indexOfFirstLocalVariable = 
			localVariables.getIndexOfFirstLocalVariable();
		...
		for (int i=indexOfFirstLocalVariable; i<nbrOfLocalVariables; ++i)
		{
			...
			lv.declarationFlag = 
				lv.exceptionOrReturnAddress ? DECLARED : NOT_DECLARED;
		}
	}
	
	private static void CreateSynchronizedBlock(
			ReferenceMap referenceMap, ClassFile classFile, 
			...
			FastCodeException fce)
	{
		...
		if (fce.type == FastConstants.TYPE_118_FINALLY)
		{
			...
			while (index-- > tryFromIndex)
			{
				...
				int jumpOffset = instruction.offset + ((Jsr)instruction).value;
				list.remove(index);
			...
			synchronizedBlockJumpOffset = SearchMinusJumpOffset(
			...
			ExecuteReconstructors(
				referenceMap, classFile, instructions, localVariables);
			...
			// 
			int branch = 1;
			if ((fastSynchronizedOffset != -1) && 
				(synchronizedBlockJumpOffset != -1))
				branch = synchronizedBlockJumpOffset-fastSynchronizedOffset;
			
			FastSynchronized fastSynchronized = new FastSynchronized(
				FastConstants.SYNCHRONIZED, fastSynchronizedOffset, 
				fastSynchronizedLineNumber, branch, instructions);
		...
		else if (fce.type == FastConstants.TYPE_118_SYNCHRONIZED_DOUBLE)
		{
		    ...
		    //  11: monitorenter
				//  12: aload_0
				//  13: invokespecial 8	TestSynchronize:getMonitor	()Ljava/lang/Object;
				//  16: astore 4
				//  18: aload 4
				//  20: monitorenter
					//  21: getstatic 10	java/lang/System:out	Ljava/io/PrintStream;
					//  24: ldc 2
					//  26: invokevirtual 11	java/io/PrintStream:println	(Ljava/lang/String;)V
					//  29: iconst_1
					//  30: istore_3
					//  31: jsr +12 -> 43
					//  34: jsr +19 -> 53
			...
			FastSynchronized fastSynchronized = new FastSynchronized(
				FastConstants.SYNCHRONIZED, fastSynchronizedOffset, 
				menter.lineNumber, branch, instructions);
			fastSynchronized.monitor = monitor; 
		...
		else if (instruction.opcode == ByteCodeConstants.MONITOREXIT)
		{
			if (list.get(--index).opcode == ByteCodeConstants.MONITORENTER)
			{
				...
				if ((gi.opcode != ByteCodeConstants.GOTO) || 
					(((Goto)gi).GetJumpOffset() != fce.afterOffset))				
					instructions.add(gi);
				...
				FastSynchronized fastSynchronized = new FastSynchronized(
					...
					instruction.lineNumber, 1, instructions);
				fastSynchronized.monitor = monitor; 
			...
			else
			{
				...
				case ByteCodeConstants.MONITORENTER:
					...
					AssignmentInstruction ai = 
						(AssignmentInstruction)menter.objectref;
				...
				while (true)
				{
					...
					if (instruction.opcode == ByteCodeConstants.MONITOREXIT)
					{
						...
						{
							LoadInstruction li = 
								(LoadInstruction)mexit.objectref;
				...
				if ((index+1 < list.size()) && 
					(list.get(index+1).opcode == ByteCodeConstants.XRETURN))
				{
					// Si l'instruction retournée possède un offset inferieur a
					...
					Instruction value = 
						((ReturnInstruction)list.get(index+1)).valueref;
				...
				FastSynchronized fastSynchronized = new FastSynchronized(
					FastConstants.SYNCHRONIZED, instruction.offset, 
					menter.lineNumber, branch, instructions);
				fastSynchronized.monitor = monitor; 
		...
		else
		{
			...
			int monitorLocalVariableIndex = 
				GetMonitorLocalVariableIndex(list, index);
			...
			if (lenght > 0)
			{
				...
				RemoveAllMonitorExitInstructions(
					instructions, lenght, monitorLocalVariableIndex);
			...
			if (monitorLocalVariableIndex != -1)
			{
				...
				localVariables.removeLocalVariableWithIndexAndOffset(
					monitorLocalVariableIndex, menter.offset);
			}				
			...
			FastSynchronized fastSynchronized = new FastSynchronized(
				FastConstants.SYNCHRONIZED, lastOffset, 
	...
	private static Instruction FormatAndExtractMonitor(
	...
	private static void RemoveAllMonitorExitInstructions(
		List<Instruction> instructions, int lenght, 
		int monitorLocalVariableIndex)
	{
		int index = lenght;
		...
		{				
			...
			else if (instruction.opcode == FastConstants.TRY)
			{
				...
				RemoveAllMonitorExitInstructions(
					ft.instructions, ft.instructions.size(), 
				...
				while (i-- > 0)
				{
					...
					RemoveAllMonitorExitInstructions(
						fc.instructions, fc.instructions.size(),
						monitorLocalVariableIndex);
				}
				
				if (ft.finallyInstructions != null)
					RemoveAllMonitorExitInstructions(
						ft.finallyInstructions, ft.finallyInstructions.size(),
						monitorLocalVariableIndex);
			}			
			else if (instruction.opcode == FastConstants.SYNCHRONIZED)
			{
				...
				RemoveAllMonitorExitInstructions(
					fsy.instructions, fsy.instructions.size(), 
					monitorLocalVariableIndex);
			}
		}
	}
	
	private static int GetMonitorLocalVariableIndex(
			List<Instruction> list, int index)
	{
		MonitorEnter menter = (MonitorEnter)list.get(index);
		...
		case ByteCodeConstants.ASSIGNMENT:
			...
			if (i.opcode == ByteCodeConstants.ASTORE)
				return ((AStore)i).index;
		default:
			return -1;
		}				
	}
	
	private static void CreateFastTry(
			ReferenceMap referenceMap, ClassFile classFile, Method method, 
			List<Instruction> list, LocalVariables localVariables, 
			FastCodeException fce, int returnOffset)
		throws Exception
	{
		...
		if ((afterListOffset == -1) || 
			(afterListOffset > list.get(lastIndex).offset))
			index = lastIndex;
		else
			index = InstructionUtil.getIndexForOffset(list, afterListOffset) - 1;
		...
		if (fce.catches != null)
		{
			catches = new ArrayList<FastCatch>(fce.catches.size());
			
			for (int i=fce.catches.size()-1; i>=0; --i)
			{
				...
				int tryJumpOffsetTmp = SearchMinusJumpOffset(
					instructions, 0, instructions.size(), 
					fce.tryFromOffset, fce.afterOffset);
				...
				if (el != null)
				{
					...
					catches.add(0, new FastCatch(
						offset, el.exceptionNameIndex, el.index, instructions));
					
					afterListOffset = instructions.get(0).offset;	
				}
				else
				{
					System.err.println("CreateFastTry: Unexpected instruction");
				}
			}
		}

		// Extract try blockes
		fce.tryToOffset = afterListOffset;		
		...
		int tryJumpOffsetTmp = SearchMinusJumpOffset(
			tryInstructions, 0, tryInstructions.size(), 
			fce.tryFromOffset, fce.afterOffset);
		if (tryJumpOffsetTmp != -1)
		{
			if (tryJumpOffset == -1)
		...
		FastTry fastTry = new FastTry(
			FastConstants.TRY, lastOffset, lineNumber, tryJumpOffset-lastOffset, 
			tryInstructions, catches, finallyInstructions);

		// Reduce lists of instructions
		FastCodeExceptionAnalyzer.FormatFastTry(fce, fastTry, returnOffset);
				
		// Analyze lists of instructions	
		ExecuteReconstructors(
			referenceMap, classFile, tryInstructions, localVariables);
		...
		if (catches != null)
		{
			for (int j=0; j<catches.size(); ++j)
			{
				...
				List<Instruction> instructions = fc.instructions;
				ExecuteReconstructors(
					referenceMap, classFile, instructions, localVariables);
			}
		}
		...
		if (finallyInstructions != null)	
			ExecuteReconstructors(
				referenceMap, classFile, finallyInstructions, localVariables);
		
		// Store new FastTry instruction
		list.set(index+1, fastTry);
	}

	private static ExceptionLoad SearchExceptionLoadInstruction(
		...
		for (int i=0; i<length; i++)
		{
			Instruction instruction = 
				SearchInstructionByOpcodeVisitor.visit(
					instructions.get(i), ByteCodeConstants.EXCEPTIONLOAD);
	 ...
	 *                |    |   |                                       |  |
	 *                |    |   beforeListOffset                        |  |
	 * Offsets        |    loopEntryOffset                 endLoopOffset  |
	 *                beforeLoopEntryOffset                 afterLoopOffset
	 */
	private static void ExecuteReconstructors(
			ReferenceMap referenceMap, ClassFile classFile, 
			List<Instruction> list, LocalVariables localVariables)
	{
		...
		// A executer apres 'AssignmentInstructionReconstructor', 'IfGotoToIfReconstructor'
	...
	private static void RemoveNoJumpGotoInstruction(
		List<Instruction> list, int afterListOffset)
	{
		int index = list.size();
		...
		if (instruction.opcode == ByteCodeConstants.GOTO)
		{
			...
			if ((branch >= 0) && 
				(instruction.offset + branch <= afterListOffset))
				list.remove(index);
		}
		...
		while (index-- > 0)
		{
			...
			if (instruction.opcode == ByteCodeConstants.GOTO)
			{
				int branch = ((Goto)instruction).branch;				
				if ((branch >= 0) && 
					(instruction.offset + branch <= lastInstructionOffset))
					list.remove(index);
			}
	 ...
	 *     
	 *      
	 * beforeLoopEntryOffset & loopEntryOffset: utile pour la generation d'instructions 'continue'
	 * beforeListOffset: utile pour la generation de declarations de variable
	 * endLoopOffset & afterLoopOffset: utile pour la generation d'instructions 'break'
	 * afterListOffset: utile pour la generation d'instructions 'if-else' = lastBodyWhileLoop.offset
	 *      
	 * WHILE
	 *  instruction avant boucle | 
	 *                      goto | beforeSubListOffset
	 *              instructions | 
	 *               instruction | beforeLoopEntryOffset
	 *         if a saut negatif | loopEntryOffset, endLoopOffset, afterListOffset
	 *  instruction apres boucle | afterLoopOffset
	 * 
	 * DO_WHILE
	 *  instruction avant boucle | beforeListOffset 
	 *              instructions | 
	 *               instruction | beforeLoopEntryOffset
	 *         if a saut negatif | loopEntryOffset, endLoopOffset, afterListOffset
	 *  instruction apres boucle | afterLoopOffset
	 ...
	 * FOR
	 *  instruction avant boucle | 
	 *                      goto | beforeListOffset
	 *              instructions | 
	 *               instruction | beforeLoopEntryOffset
	 *                      iinc | loopEntryOffset, afterListOffset
	 *         if a saut negatif | endLoopOffset
	 *  instruction apres boucle | afterLoopOffset
	 * 
	 * 
	 * INFINITE_LOOP
	 ...
	 *              instructions | 
	 *               instruction | beforeLoopEntryOffset
	 *       goto a saut negatif | loopEntryOffset, endLoopOffset, afterListOffset
	 *  instruction apres boucle | afterLoopOffset
	 *  
	 */
	private static void AnalyzeList(
		ClassFile classFile, Method method, List<Instruction> list, 
		LocalVariables localVariables, IntSet offsetLabelSet, 
		int beforeLoopEntryOffset, int loopEntryOffset, 
		...
		int breakOffset, int returnOffset)
	{
		...
		CreateLoops(
			classFile, method, list, localVariables, offsetLabelSet, 
			beforeLoopEntryOffset, loopEntryOffset, beforeListOffset, 
			afterListOffset, returnOffset);
		
		// Create switch
		CreateSwitch(
			classFile, method, list, localVariables, offsetLabelSet, 
			beforeLoopEntryOffset, loopEntryOffset, afterListOffset, 
			returnOffset);	

		AnalyzeTryAndSynchronized(
			classFile, method, list, localVariables, offsetLabelSet, 
			beforeLoopEntryOffset, loopEntryOffset, beforeListOffset, 
    	...
    	// 2) DEPLACER CETTE METHODE APRES L'APPEL A FastInstructionListBuilder.Build(...)
    	RemoveNoJumpGotoInstruction(list, afterListOffset);
				
		// Create labeled 'break'
		CreateBreakAndContinue(
			method, list, offsetLabelSet, beforeLoopEntryOffset, 
			loopEntryOffset, breakOffset, returnOffset);
	...
	private static void AnalyzeTryAndSynchronized(
		ClassFile classFile, Method method, List<Instruction> list, 
		LocalVariables localVariables, IntSet offsetLabelSet, 
		int beforeLoopEntryOffset, int loopEntryOffset, 
		int beforeListOffset, int afterListOffset, 
		int breakOffset, int returnOffset)
	{
		...
		while (index-- > 0)
		{
			...
			case FastConstants.TRY:
				{
					FastTry ft = (FastTry)instruction;
					int tmpBeforeListOffset = (index > 0) ? 
							list.get(index-1).offset : beforeListOffset;
					
					// Try block
					AnalyzeList(
			    		classFile, method, ft.instructions, 
			    		localVariables, offsetLabelSet, beforeLoopEntryOffset, 
			    		loopEntryOffset, tmpBeforeListOffset, afterListOffset, 
			    		breakOffset, returnOffset);
					...
					{
						AnalyzeList(
				    		classFile, method, ft.catches.get(i).instructions, 
				    		localVariables, offsetLabelSet, beforeLoopEntryOffset, 
				    		loopEntryOffset, tmpBeforeListOffset, afterListOffset, 
				    		breakOffset, returnOffset);
					}
					...
					{
						AnalyzeList(
				    		classFile, method, ft.finallyInstructions, 
				    		localVariables, offsetLabelSet, beforeLoopEntryOffset, 
				    		loopEntryOffset, tmpBeforeListOffset, afterListOffset, 
				    		breakOffset, returnOffset);
					}
			...
			case FastConstants.SYNCHRONIZED:
				{
					...
					AnalyzeList(
			    		classFile, method, fs.instructions, localVariables, 
			    		offsetLabelSet, beforeLoopEntryOffset, loopEntryOffset,
			    		tmpBeforeListOffset, afterListOffset, 
			    		breakOffset, returnOffset);
				}
				break;
			}
	...
	/*
	 * Strategie :
	 * 1) Les instructions 'store' et 'for' sont passées en revue. Si elles 
	 *    referencent une variables locales non encore declarée et dont la 
	 *    portée est incluse à la liste, une declaration est insérée.
	 * 2) Le tableau des variables locales est passé en revue.
	 *    Pour toutes variables locales non encore declarées et dont la portée
	 *    est incluse à la liste courante, on declare les variables en debut de
	 *    bloc.
	 */
	private static void AddDeclarations(
			List<Instruction> list, LocalVariables localVariables, 
			int beforeListOffset)
	{
		...
		if (listLength > 0)
		{
			...
			int lastOffset = list.get(listLength-1).offset;
			...
			for (int i=0; i<listLength; i++)
			{
				...
				case ByteCodeConstants.STORE:
					...
					if ((lv != null) && (lv.declarationFlag == NOT_DECLARED) && 
						(beforeListOffset < lv.start_pc) && 
						(lv.start_pc+lv.length-1 <= lastOffset))
					{
						list.set(i, new FastDeclaration(
							FastConstants.DECLARE, si.offset, si.lineNumber, 
							si.index, si));
					...
					if (ff.init != null)
					{
						...
						case FastConstants.STORE:
							...
							if ((lv != null) && (lv.declarationFlag == NOT_DECLARED) && 
								...
								(lv.start_pc+lv.length-1 <= lastOffset))
							{
								ff.init = new FastDeclaration(
									FastConstants.DECLARE, si.offset, 
									si.lineNumber, si.index, si);
								lv.declarationFlag = DECLARED;
								UpdateNewAndInitArrayInstruction(si);
							}
						}
			...
			// 2) Ajout de declaration pour toutes variables non encore declarées
			...
			for (int i=0; i<lvLength; i++)
			{
				...
				if ((lv.declarationFlag == NOT_DECLARED) && 
					(beforeListOffset < lv.start_pc) && 
					(lv.start_pc+lv.length-1 <= lastOffset))
				{
					int indexForNewDeclaration = 
						InstructionUtil.getIndexForOffset(list, lv.start_pc);
					...
					list.add(indexForNewDeclaration, new FastDeclaration(
						FastConstants.DECLARE, lv.start_pc,
	...
	private static void CreateBreakAndContinue(
		Method method, List<Instruction> list,	IntSet offsetLabelSet, 
		int beforeLoopEntryOffset, int loopEntryOffset, 
		int breakOffset, int returnOffset)
	{
		int length = list.size();
			...
			case FastConstants.COMPLEXIF:
				{
					...
					if ((beforeLoopEntryOffset < jumpOffset) && 
						(jumpOffset <= loopEntryOffset))
					{
						list.set(index, new FastInstruction(
							FastConstants.IF_CONTINUE, bi.offset, 
					...
					else if (JumpTo(method, breakOffset, jumpOffset))
					{
						list.set(index, new FastInstruction(
							FastConstants.IF_BREAK, bi.offset, 
							bi.lineNumber, bi));	
					}
					else
					{
						...
						if (JumpTo(method, jumpOffset, returnOffset))
						{
							List<Instruction> instructions = 
								new ArrayList<Instruction>(1);
							instructions.add(new Return(
						...
						else
						{
							...
							list.set(index, new FastInstruction(
								FastConstants.IF_LABELED_BREAK, bi.offset, 
								bi.lineNumber, bi));		
						}
					}
				}
			...
			case FastConstants.GOTO:
				{
					...
					{
						list.set(index, new FastInstruction(
							FastConstants.GOTO_CONTINUE, g.offset, 
							g.lineNumber, null));	
					}
					else if (JumpTo(method, breakOffset, jumpOffset))
					{
						list.set(index, new FastInstruction(
							FastConstants.GOTO_BREAK, g.offset, 
							g.lineNumber, null));	
					}
					else
					{
						...
						if (JumpTo(method, jumpOffset, returnOffset))
						{
							list.set(index, new Return(
								ByteCodeConstants.RETURN, g.offset, g.lineNumber));	
						}
						else 
						{
							offsetLabelSet.add(jumpOffset);
							list.set(index, new FastInstruction(
								FastConstants.GOTO_LABELED_BREAK, instruction.offset, 
								instruction.lineNumber, instruction));	
						}
					}
				}
				break;
	...
	private static boolean JumpTo(Method method, int offset, int targetOffset)
	{
		if (offset != -1)
		{
	...
	private static int UnoptimizeIfElseInLoop(
		ClassFile classFile, Method method, List<Instruction> list, 
		LocalVariables localVariables, IntSet offsetLabelSet, 
		int beforeListOffset, int afterListOffset, int returnOffset,
		int offset, int jumpOffset, int index)
	{
		int firstLoopInstructionIndex = 
			InstructionUtil.getIndexForOffset(list, jumpOffset);
				...
				if (afterLoopJumpOffset > afterLoopInstructionOffset)
				{
					...
					if (afterLoopInstructionIndex != -1)
					{
								...
								offset, lastInstructionoffset))
						{
						    ...
						    //  564: if (nodeAgentSearch.getCanonicalName().indexOf(this.asName) <= 0) goto 532; <---
						    //  568: found = true;
						    //  569: goto 572;
						    //  572: ...
							// Pour:
							...
							//   if (nodeAgentSearch.getCanonicalName().indexOf(this.asName) > 0) {
							//    found = true;
							//    break;
							//   }
							//  }
							...
							Instruction lastInstruction = 
								list.get(afterLoopInstructionIndex-1);
							...
							Goto newGi = new Goto(
									...
									Instruction.UNKNOWN_LINE_NUMBER, 
									jumpOffset-lastInstruction.offset);
							list.add(afterLoopInstructionIndex, newGi);
							
							return AnalyzeBackGoto(
								classFile, method, list, localVariables, 
								offsetLabelSet, beforeListOffset, 
								afterLoopJumpOffset, returnOffset, 
								afterLoopInstructionIndex, newGi, jumpOffset);
						}
					}
				}
			}
	...
	private static int UnoptimizeIfiniteLoop(
		ClassFile classFile, Method method, List<Instruction> list, 
		LocalVariables localVariables, IntSet offsetLabelSet, 
		int beforeListOffset, int afterListOffset, int returnOffset,
		BranchInstruction bi, int jumpOffset, int jumpIndex)
	{
		// Original:
		//   8: ...
	    //  18: ...
		...
		int afterGotoOffset = (jumpIndex+2 >= length) ? 
			afterListOffset : list.get(jumpIndex+2).offset;
		
		Goto g = (Goto)instruction;
		int jumpGotoOffset = g.GetJumpOffset();
		
		if ((g.offset >= jumpGotoOffset) || 
			(jumpGotoOffset > afterGotoOffset))
			return -1;
		...
		Goto newGoto = new Goto(
			ByteCodeConstants.GOTO, newGotoOffset,
			Instruction.UNKNOWN_LINE_NUMBER, 
			jumpOffset-newGotoOffset);
		list.add(jumpIndex+2, newGoto);
		
		return AnalyzeBackGoto(
			classFile, method, list, localVariables, 
			offsetLabelSet, beforeListOffset, jumpGotoOffset, 
			returnOffset, jumpIndex+2, newGoto, jumpOffset);
	}
	
	@Deprecated
	private static int UnoptimizeIfiniteLoop_old(
		ClassFile classFile, Method method, List<Instruction> list, 
		LocalVariables localVariables, IntSet offsetLabelSet, 
		int beforeListOffset, int afterListOffset, int returnOffset,
		int offset, int jumpOffset, int jumpIndex)
	{
		// Une instruction, dans le corps de la boucle, saute-t-elle vers
		// 'jumpOffset' ?
		boolean found = false;
		int index = jumpIndex;
		...
		while (index-- > 0)
		{
			...
			case FastConstants.SYNCHRONIZED:
				BranchInstruction bi = (BranchInstruction)instruction;
				if (bi.GetJumpOffset() == jumpOffset)
				{
					found = true;
					index = 0;
					break;
		...
		// Recherche de la fin de boucle
		int length = list.size();
		
		while (jumpIndex < length)
		{
			Instruction instruction = list.get(jumpIndex);
			int opcode = instruction.opcode;
			
			if ((opcode == ByteCodeConstants.ATHROW) ||
				(opcode == ByteCodeConstants.RETURN) ||
				(opcode == ByteCodeConstants.XRETURN))
			{
				...
				break;
					 ...
					 (opcode == ByteCodeConstants.IF) ||
					 (opcode == ByteCodeConstants.IFXNULL))
			{
				int branch = ((BranchInstruction)instruction).branch;
				if (branch > 0)
				{
					// Saut positif
					int jumpOffsetTmp = instruction.offset + branch;
					while (++jumpIndex < length)	
					{
						if (list.get(jumpIndex).offset >= jumpOffsetTmp)
						{
							--jumpIndex;
							break;
						}
		...
		if (jumpIndex < length)
		{
			offset = list.get(jumpIndex).offset;
			afterLoopJumpOffset = list.get(jumpIndex).offset;
		}
		else
		{
			offset = list.get(length-1).offset;
			afterLoopJumpOffset = afterListOffset;
		}
		...
		// l'instruction precedente.
		Goto newGi = new Goto(
			ByteCodeConstants.GOTO, offset,
			Instruction.UNKNOWN_LINE_NUMBER, 
			jumpOffset-offset);
		list.add(jumpIndex, newGi);
		
		return AnalyzeBackGoto(
			classFile, method, list, localVariables, 
			offsetLabelSet, beforeListOffset, afterLoopJumpOffset, 
			returnOffset, jumpIndex, newGi, jumpOffset);
	}
			...
			case FastConstants.COMPLEXIF:
				{
					...
					if (bi.branch < 0)
					{
						...
						if ((beforeListOffset < jumpOffset) &&
							((beforeLoopEntryOffset >= jumpOffset) || 
							 (jumpOffset > loopEntryOffset)))
						{
							int newIndex = UnoptimizeIfElseInLoop(
								classFile, method, list, localVariables, 
								offsetLabelSet, beforeListOffset, 
								afterListOffset, returnOffset,
								bi.offset, jumpOffset, index);

							if (newIndex == -1)
							{
								newIndex = UnoptimizeIfiniteLoop(
									classFile, method, list, localVariables, 
									offsetLabelSet, beforeListOffset, 
									afterListOffset, returnOffset,
									bi, jumpOffset, index);
					...
					if (gi.branch < 0)
					{
							 ...
							 (jumpOffset > loopEntryOffset)))
						{
							int newIndex = UnoptimizeIfElseInLoop(
								...
								offsetLabelSet, beforeListOffset, 
								afterListOffset, returnOffset,
								gi.offset, jumpOffset, index);
							
							if (newIndex == -1)
							{
								index = AnalyzeBackGoto(
									classFile, method, list, localVariables, 
									offsetLabelSet, beforeListOffset, 
									gi.offset, returnOffset, index, gi, 
									jumpOffset);
							}
							else
							{
			...
			case FastConstants.SYNCHRONIZED:
				{
					...
					if (fl.instructions.size() > 0)
					{
						...
						if ((jumpOffset != -1) && 
							(previousOffset >= jumpOffset) &&
							(beforeListOffset < jumpOffset) &&
							((beforeLoopEntryOffset >= jumpOffset) || 
							  (jumpOffset > loopEntryOffset)))
						{
							fl.branch = 1;	
							int afterSubListOffset = (index+1 < list.size()) ? 
								list.get(index+1).offset : afterListOffset;
							index = AnalyzeBackGoto(
								classFile, method, list, localVariables, 
								offsetLabelSet, beforeListOffset, 
								afterSubListOffset, returnOffset, index, fl, 
								jumpOffset);
						}
					}
				}
	...
	private static boolean UnoptimizeLoopInLoop(
		List<Instruction> list, int beforeListOffset, 
		int index, Instruction instruction)
	{
		...
		case FastConstants.COMPLEXIF:
			...
			if (bi.opcode == ByteCodeConstants.GOTO)
			{
				...
				// 1) Create 'goto'
				list.add(indexBi+1, new Goto(
					ByteCodeConstants.GOTO, bi.offset+1,
					Instruction.UNKNOWN_LINE_NUMBER,
			...
			else
			{
				...
				//   '--->+ if ----'              '--->+ if ------'  
				//        |                            |
				if ((target.opcode == ByteCodeConstants.GOTO) && 
					(((Goto)target).GetJumpOffset() == jumpOffset2))
				{
					// 'goto' exists 
				...
				else
				{
					// Goto does not exist
					// 1) Create 'goto'
					list.add(index+1, new Goto(
						ByteCodeConstants.GOTO, jumpOffset2-1, 
						Instruction.UNKNOWN_LINE_NUMBER,
						jumpOffset-jumpOffset2+1));
					// 2) Modify branch offset of first loop
	 ...
	 *                beforeLoopEntryOffset                 afterLoopOffset
	 */
	private static void CreateIfElse(
		ClassFile classFile, Method method, List<Instruction> list, 
		LocalVariables localVariables, IntSet offsetLabelSet, 
		int beforeLoopEntryOffset, int loopEntryOffset, 
		int afterListOffset, int breakOffset, int returnOffset)
	{
		// Create if and if-else
		int length = list.size();
		for (int index=0; index<length; index++)
		{
			...
			case FastConstants.COMPLEXIF:
				AnalyzeIfAndIfElse(
					classFile, method, list, localVariables, offsetLabelSet, 
					beforeLoopEntryOffset, loopEntryOffset, afterListOffset, 
					breakOffset, returnOffset, index, 
	...
	private static void CreateSwitch(
		...
		LocalVariables localVariables, IntSet offsetLabelSet, 
		int beforeLoopEntryOffset, int loopEntryOffset, 
		int afterListOffset, int returnOffset)
	{
		// Create switch
		for (int index=0; index<list.size(); index++)
		{
			...
			case FastConstants.LOOKUPSWITCH:
				AnalyzeLookupSwitch(
					classFile, method, list, localVariables, offsetLabelSet, 
					beforeLoopEntryOffset, loopEntryOffset, 
					afterListOffset, returnOffset, index, 
					(LookupSwitch)instruction);
				break;
				
			case FastConstants.TABLESWITCH:
				index = AnalyzeTableSwitch(
					classFile, method, list, localVariables, offsetLabelSet, 
					beforeLoopEntryOffset, loopEntryOffset, 
					afterListOffset, returnOffset, index, 
	...
	private static void RemoveLocalVariable(Method method, IndexInstruction ii)
	{
		LocalVariable lv = method.getLocalVariables()
			.searchLocalVariableWithIndexAndOffset(ii.index, ii.offset);

		if ((lv != null) && (ii.offset == lv.start_pc))
		{
			method.getLocalVariables()
				.removeLocalVariableWithIndexAndOffset(ii.index, ii.offset);
		}
	}	
	 ...
	 */
	private static int AnalyzeBackIf(
		ClassFile classFile, Method method, List<Instruction> list, 
		LocalVariables localVariables, IntSet offsetLabelSet, 
		int beforeListOffset, int returnOffset, int testIndex, Instruction test)
	{
		...
		int beforeLoopEntryOffset = (index >= 0) ?
			list.get(index).offset : beforeListOffset;
		...
		int breakOffset = SearchMinusJumpOffset(
			subList, 0, subListLength, 
			beforeListOffset, test.offset);
		
		// Search 'goto' before 'while' loop
		Goto gotoBeforeLoop = null; 
		...
		if (index >= 0)
		{
			...
			{
				Goto gi = (Goto)instruction;
				int offset = gi.GetJumpOffset();
				int lastBodyOffset = 
					(subList.size() > 0) ? subList.get(0).offset : gi.offset;
		...
		if (gotoBeforeLoop != null)
		{
			...
			if (subListLength > 0)
			{
				...
				if (subListLength > 1)
				{
					...
					if (!InstructionUtil.CheckNoJumpToInterval(
						subList, 0, subListLength, 
						lastBodyLoop.offset, test.offset))
					{
			...
			int typeLoop = GetLoopType(
				beforeLoop, test, beforeLastBodyLoop, lastBodyLoop);
			...
			case 2: // while (test)				
				...
				{						
					...
					AnalyzeList(
						classFile, method, subList, localVariables, 
						offsetLabelSet, beforeLoopEntryOffset, 
						test.offset, gotoBeforeLoop.offset, 
						test.offset, breakOffset, returnOffset);
				}
				...
				list.set(++index, new FastTestList(
					FastConstants.WHILE, test.offset, 
					test.lineNumber, branch, test, subList));
				break;
			case 3: // for (beforeLoop; test;)
				...
				CreateForLoopCase1(
					...
					beforeLoop, test, subList, breakOffset);
				break;
			case 6: // for (; test; lastBodyLoop)
				...
				{						
					...
					AnalyzeList(
						classFile, method, subList, localVariables, 
						offsetLabelSet, 
						beforeLastBodyLoop.offset, lastBodyLoop.offset, 
						gotoBeforeLoop.offset, lastBodyLoop.offset, 
						breakOffset, returnOffset);
					...
					list.set(++index, new FastFor(
						FastConstants.FOR, test.offset, test.lineNumber, 
						branch, null, 
						test, lastBodyLoop, subList));	
				}
				else
				{	
					if (subListLength == 1)
			...
			case 7: // for (beforeLoop; test; lastBodyLoop)
				...
				index = CreateForLoopCase3(
					classFile, method, list, index, beforeLoop, test, 
					lastBodyLoop, subList, breakOffset);
				break;
			default:
		...
		else
		{
			if (subListLength > 0)
			{
				Collections.reverse(subList);		
				AnalyzeList(
					classFile, method, subList, localVariables, offsetLabelSet, 
					beforeLoopEntryOffset, test.offset, beforeListOffset, 
					test.offset, breakOffset, returnOffset);
				...
				list.set(++index, new FastTestList(
					FastConstants.DO_WHILE, test.offset, 
					Instruction.UNKNOWN_LINE_NUMBER, branch, 
					test, subList));
			}		
			else
			{
				// 'do-while' avec une liste d'instructions vide devient un 'while'.
				list.set(++index, new FastTestList(
					FastConstants.WHILE, test.offset, 
					test.lineNumber, 1, test, null));
			}
		}
	...
	private static int SearchMinusJumpOffset(
		List<Instruction> list, int fromIndex, int toIndex,
		int beforeListOffset, int lastListOffset)
	{
		...
		while (index-- > fromIndex)
		{
			...
			case FastConstants.COMPLEXIF:
				...
				if ((jumpOffset != -1) &&
					((jumpOffset <= beforeListOffset) ||
					 (lastListOffset < jumpOffset)))
				{
			...
			case FastConstants.SYNCHRONIZED:
				...
				{
					jumpOffset = SearchMinusJumpOffset(
						instructions, 0, instructions.size(),
			...
			case FastConstants.TRY:
				...
				while (i-- > 0)
				{
					...
					jumpOffset = SearchMinusJumpOffset(
						catchInstructions, 0, catchInstructions.size(),
						beforeListOffset, lastListOffset);	
			...
			case FastConstants.SWITCH_STRING:
				...
				while (i-- > 0)
				{
					...
					{
						jumpOffset = SearchMinusJumpOffset(
							caseInstructions, 0, caseInstructions.size(),
							beforeListOffset, lastListOffset);	
	...
	private static int GetMaxOffset(
			Instruction beforeWhileLoop, Instruction test)
	{
		return 
			(beforeWhileLoop.offset > test.offset) ? 
			beforeWhileLoop.offset : test.offset;
	}
		
	private static int GetMaxOffset(
			Instruction beforeWhileLoop, Instruction test, 
			Instruction lastBodyWhileLoop)
	{
		int offset = GetMaxOffset(beforeWhileLoop, test);
		
		return 
			(offset > lastBodyWhileLoop.offset) ? 
			offset : lastBodyWhileLoop.offset;
	}
	...
	private static Instruction CreateForEachVariableInstruction(Instruction i)
	{
		...
		case FastConstants.ASTORE:		
			return new ALoad(
				FastConstants.ALOAD, i.offset, i.lineNumber, ((AStore)i).index);
		case FastConstants.ISTORE:
			return new ILoad(
				FastConstants.ILOAD, i.offset, i.lineNumber, ((IStore)i).index);
		case FastConstants.STORE:
			return new LoadInstruction(
				FastConstants.LOAD, i.offset, i.lineNumber, 
				((StoreInstruction)i).index, 
	...
	private static void CreateForLoopCase1(
		ClassFile classFile, Method method, List<Instruction> list, 
		int beforeWhileLoopIndex, Instruction beforeWhileLoop, 
		Instruction test, List<Instruction> subList, int breakOffset)
	{
		...
		if (IsAForEachIteratorPattern(
			classFile, method, beforeWhileLoop, test, subList))
		{
			Instruction variable = 
				CreateForEachVariableInstruction(subList.remove(0));		
			
			InvokeNoStaticInstruction insi = 
				(InvokeNoStaticInstruction)
					(((AStore)beforeWhileLoop).valueref);
			...
			list.set(beforeWhileLoopIndex, new FastForEach(
				FastConstants.FOREACH, forLoopOffset, 
				beforeWhileLoop.lineNumber, branch, 
				variable, values, subList));								
		}
		else
		{
			list.set(beforeWhileLoopIndex, new FastFor(
				FastConstants.FOR, forLoopOffset, 
				beforeWhileLoop.lineNumber, branch, 
				beforeWhileLoop, test, null, subList));
		}
	}

	private static int CreateForLoopCase3(
			ClassFile classFile, Method method, List<Instruction> list, 
			int beforeWhileLoopIndex, Instruction beforeWhileLoop, 
			Instruction test, Instruction lastBodyWhileLoop, 
			List<Instruction> subList, int breakOffset)
	{
		int forLoopOffset = 
			GetMaxOffset(beforeWhileLoop, test, lastBodyWhileLoop);
		...
		// Is a for-each pattern ?
		switch (GetForEachArrayPatternType(
			classFile, beforeWhileLoop, test, 
			lastBodyWhileLoop, list, beforeWhileLoopIndex, 
			subList))
		{			
		case 1: // SUN 1.5
			{
				...
				StoreInstruction beforeBeforeWhileLoop = 
					(StoreInstruction)list.remove(--beforeWhileLoopIndex);
				AssignmentInstruction ai = 
					(AssignmentInstruction)((ArrayLength)
						beforeBeforeWhileLoop.valueref).arrayref;	
		...
		case 2: // SUN 1.6
			{
				...
				StoreInstruction beforeBeforeBeforeWhileLoop = 
					(StoreInstruction)list.remove(--beforeWhileLoopIndex);
		...
		default:
			{
				list.set(beforeWhileLoopIndex, new FastFor(
					FastConstants.FOR, forLoopOffset, 
					beforeWhileLoop.lineNumber, branch, 
					beforeWhileLoop, test, lastBodyWhileLoop, subList));	
			}
		}
	...
	/* Pattern:
	 *  7: List strings = new ArrayList();
     * 44: for (Iterator localIterator = strings.iterator(); localIterator.hasNext(); )
     * {
     *   29: String s = (String)localIterator.next();
     *   34: System.out.println(s);
     * }
	 */
	private static boolean IsAForEachIteratorPattern(
			ClassFile classFile, Method method, Instruction init, 
			Instruction test, List<Instruction> subList)
	{
		...
		if ((astoreIterator.valueref.opcode != ByteCodeConstants.INVOKEINTERFACE) &&
			...
			return false;
		LocalVariable lv = 
			method.getLocalVariables().getLocalVariableWithIndexAndOffset(
					astoreIterator.index, astoreIterator.offset);
		if ((lv == null) || (lv.signature_index == 0))
			return false;
		ConstantPool constants = classFile.getConstantPool();
		InvokeNoStaticInstruction insi = 
			(InvokeNoStaticInstruction)astoreIterator.valueref;
		...
		if (! "iterator".equals(iteratorMethodName))
			return false;
		String iteratorMethodDescriptor = 
		...
		if (! "hasNext".equals(hasNextMethodName))
			return false;
		String hasNextMethodDescriptor = 
			constants.getConstantUtf8(cnat.descriptor_index);
     ...
     *        1: Pattern SUN 1.5
	 */
	private static int GetForEachArraySun15PatternType(
		...
		Instruction firstInstruction, StoreInstruction siLenght)
	{
		...
		if ((! ai.operator.equals("=")) || 
			(ai.value1.opcode != ByteCodeConstants.ASTORE))
			return 0;	
		...
		if ((ifcmp.value1.opcode != ByteCodeConstants.ILOAD) || 
			(ifcmp.value2.opcode != ByteCodeConstants.ILOAD) ||
			( ((ILoad)ifcmp.value1).index != siIndex.index ) ||
			( ((ILoad)ifcmp.value2).index != siLenght.index ))
			return 0;		
		
		// Test 'inc' instruction: ++i
		if ((inc.opcode != ByteCodeConstants.IINC) || 
		    (((IInc)inc).index != siIndex.index) || 
		    (((IInc)inc).count != 1))
			return 0;	
		...
		if ((firstInstruction.opcode != FastConstants.STORE) &&
			(firstInstruction.opcode != FastConstants.ASTORE) &&
		...
		if ((ali.arrayref.opcode != ByteCodeConstants.ALOAD) || 
			(ali.indexref.opcode != ByteCodeConstants.ILOAD) ||
     ...
     *        2: Pattern SUN 1.6
	 */
	private static int GetForEachArraySun16PatternType(
		Instruction init, Instruction test, Instruction inc, 
		Instruction firstInstruction, StoreInstruction siLenght, 
		Instruction beforeBeforeForInstruction)
	{
		...
		if (beforeBeforeForInstruction.opcode != ByteCodeConstants.ASTORE)
			return 0;	
		StoreInstruction siTmpArray = 
			(StoreInstruction)beforeBeforeForInstruction;
     ...
     *        3: Pattern IBM
	 */	
	private static int GetForEachArrayIbmPatternType(
		ClassFile classFile, Instruction init, Instruction test, 
		Instruction inc, List<Instruction> list, int beforeWhileLoopIndex, 
		Instruction firstInstruction, StoreInstruction siIndex)
	{
     ...
     *        3: Pattern IBM
	 */
	private static int GetForEachArrayPatternType(
			ClassFile classFile, Instruction init, 
			Instruction test, Instruction inc, List<Instruction> list, 
			int beforeWhileLoopIndex, List<Instruction> subList)
	{
		// Tests: (Java 5 or later) + (Not empty sub list)
		if ((classFile.getMajorVersion() < 49) || (beforeWhileLoopIndex == 0) || 
			(subList.size() == 0))
			return 0;
		...
		if (si.valueref.opcode == ByteCodeConstants.ARRAYLENGTH)
		{
			...
			if (al.arrayref.opcode == ByteCodeConstants.ASSIGNMENT)		
			{
				return GetForEachArraySun15PatternType(
					init, test, inc, firstInstruction, si);
			}
			else if (beforeWhileLoopIndex > 1)
			{
				Instruction beforeBeforeForInstruction = 
					list.get(beforeWhileLoopIndex-2);
				return GetForEachArraySun16PatternType(
					init, test, inc, firstInstruction, si, 
		...
		if (si.valueref.opcode == ByteCodeConstants.ICONST)
			return GetForEachArrayIbmPatternType(
				classFile, init, test, inc, list, 
				beforeWhileLoopIndex, firstInstruction, si);	
	 ...
	 */
	private static int GetLoopType(
		Instruction	beforeLoop, Instruction test, 
		Instruction beforeLastBodyLoop, Instruction lastBodyLoop)
	{
		if (beforeLoop == null)
		{
			...
			if (test == null)
			{
				...
				else
				{
					...
					return 
						((beforeLastBodyLoop != null) && 
						(beforeLastBodyLoop.lineNumber > lastBodyLoop.lineNumber)) ?
						4 : 0;
				}
			}
			else
			{
				...
				else
				{
					...
					{
						return 
							(test.lineNumber == lastBodyLoop.lineNumber) ? 
							6 : 2;
					}
				}
			}
		...
		else
		{
			...
			if (test == null)
			{
				...
				else
				{
					...
					if (beforeLoop.lineNumber == Instruction.UNKNOWN_LINE_NUMBER)
					{
						...
						// d'affectation ou d'incrementation ? (a|d|f|i|l|s)store ou iinc ?
						switch (beforeLoop.opcode)
						{
						case FastConstants.ASTORE:
						case FastConstants.DSTORE:
						case FastConstants.FSTORE:
						case FastConstants.LSTORE:
						case FastConstants.PUTFIELD:
							return 
								(beforeLoop.opcode == lastBodyLoop.opcode) &&	
								(((IndexInstruction)beforeLoop).index == 
										((IndexInstruction)lastBodyLoop).index) 
								? 5 : 0;
						
						case FastConstants.ISTORE:
							if ((beforeLoop.opcode == lastBodyLoop.opcode) || 
								(lastBodyLoop.opcode == FastConstants.IINC))
								return 
									((IndexInstruction)beforeLoop).index == 
										((IndexInstruction)lastBodyLoop).index
										? 5 : 0;
			...
			else
			{
				...
				if (lastBodyLoop == null)
				{
					...
					{
						return 
							(beforeLoop.lineNumber == test.lineNumber) ?
							3 : 2;
					}
				}
				else
				{
					...
					if (beforeLoop.lineNumber == Instruction.UNKNOWN_LINE_NUMBER)
					{
						...
						case FastConstants.LSTORE:
						case FastConstants.PUTFIELD:
							return (beforeLoop.opcode == lastBodyLoop.opcode) &&	
										(((IndexInstruction)beforeLoop).index == 
											((IndexInstruction)lastBodyLoop).index) 
									? 7 : 2;
						...
						case FastConstants.ISTORE:
							if ((beforeLoop.opcode == lastBodyLoop.opcode) || 
								(lastBodyLoop.opcode == FastConstants.IINC))
								return ((IndexInstruction)beforeLoop).index == 
											((IndexInstruction)lastBodyLoop).index
										? 7 : 2;
					...
					else
					{
						if (beforeLoop.lineNumber == test.lineNumber)
						{
							 ...
							 */
							return 
								(beforeLoop.lineNumber == lastBodyLoop.lineNumber) ?
									7 : 3;
						}
						else
						{
	 ...
	 *                beforeLoopEntryOffset                 afterLoopOffset
	 */
	private static int AnalyzeBackGoto(
		ClassFile classFile, Method method, List<Instruction> list, 
		LocalVariables localVariables, IntSet offsetLabelSet, 
		int beforeListOffset, int afterSubListOffset, 
		int returnOffset, int jumpInstructionIndex, 
		Instruction jumpInstruction, int firstOffset)
	{
		List<Instruction> subList = new ArrayList<Instruction>();
		...
		if (subListLength > 0)
		{
			...
			int breakOffset = SearchMinusJumpOffset(
				subList, 0, subListLength,
				beforeListOffset, jumpInstruction.offset);
			...
			if (subListLength > 0)
			{
				...
				else if (subListLength > 1)
				{
					...
					if (!InstructionUtil.CheckNoJumpToInterval(
							subList, 0, subListLength, 
							lastBodyLoop.offset, jumpInstruction.offset))
					{
						// 'lastBodyLoop' ne peut pas etre l'instruction
						// d'incrementation d'une boucle 'for'
						lastBodyLoop = null;
					...
					else if (!InstructionUtil.CheckNoJumpToInterval(
							subList, 0, subListLength, 
							beforeListOffset, firstOffset))
					{
						// 'lastBodyLoop' ne peut pas etre l'instruction
						// d'incrementation d'une boucle 'for'
						lastBodyLoop = null;
			...
			case 0: // for (;;)
				{
					...
					AnalyzeList(
						classFile, method, subList, localVariables, 
						offsetLabelSet, beforeListOffset, firstBodyLoop.offset, 
						beforeListOffset, afterSubListOffset, breakOffset, 
						returnOffset);
					...
					list.set(++index, new FastList(
						FastConstants.INFINITE_LOOP, jumpInstruction.offset, 
			...
			case 1: // for (beforeLoop; ;)
				{
					...
					AnalyzeList(
						classFile, method, subList, localVariables, 
						offsetLabelSet, beforeLoop.offset, firstBodyLoop.offset, 
						beforeListOffset, afterSubListOffset, breakOffset, 
						returnOffset);
			...
			case 2: // while (test)
				{
					...
					if (subListLength > 0)
					{
						...
						AnalyzeList(
							classFile, method, subList, localVariables, 
							offsetLabelSet, beforeTestOffset, test.offset, 
							test.offset, afterSubListOffset, breakOffset, 
							returnOffset);				
					}
					...
					list.set(++index, new FastTestList(
						FastConstants.WHILE, jumpInstruction.offset, 
						test.lineNumber, branch, test, subList));
				}
				break;
			case 3: // for (beforeLoop; test;)
				{
					...
					if (subListLength > 0)
					{
						...
						AnalyzeList(
							classFile, method, subList, localVariables, 
							offsetLabelSet, beforeLoop.offset, test.offset, 
							test.offset, afterSubListOffset, breakOffset, 
							returnOffset);				
					}
			...
			case 4: // for (; ; lastBodyLoop)
				{
					...
					if (subListLength > 0)
					{
						...
						AnalyzeList(
							classFile, method, subList, localVariables, 
							offsetLabelSet, beforeLastBodyLoop.offset, 
							lastBodyLoop.offset, beforeListOffset, 
					...
					list.set(++index, new FastFor(
						FastConstants.FOR, jumpInstruction.offset, 
						lastBodyLoop.lineNumber, 
			...
			case 5: // for (beforeLoop; ; lastBodyLoop)
				...
				list.set(index, new FastFor(
					FastConstants.FOR, jumpInstruction.offset, 
					lastBodyLoop.lineNumber, branch, beforeLoop, 
					null, lastBodyLoop, subList));
				break;
			case 6: // for (; test; lastBodyLoop)
				// Remove test
				subList.remove(--subListLength);
				...
				{	
					...
					list.set(++index, new FastFor(
						FastConstants.FOR, jumpInstruction.offset, 
						lastBodyLoop.lineNumber, branch, 
						null, test, lastBodyLoop, subList));
				}
				else
				{
					if (subListLength == 1)
			...
			case 7: // for (beforeLoop; test; lastBodyLoop)
				...
				index = CreateForLoopCase3(
					classFile, method, list, index, 
					beforeLoop, test, lastBodyLoop, subList, breakOffset);	
				break;
			}
		}
		else
	 ...
	 *                beforeLoopEntryOffset                 afterLoopOffset
	 */
	private static void AnalyzeIfAndIfElse(
		ClassFile classFile, Method method, List<Instruction> list, 
		LocalVariables localVariables, IntSet offsetLabelSet, 
		int beforeLoopEntryOffset, int loopEntryOffset, int afterListOffset, 
		int breakOffset, int returnOffset, int testIndex, 
		ConditionalBranchInstruction test)
	{
		...
		if (index < length)
		{
			// Construct if block instructions
			List<Instruction> subList = new ArrayList<Instruction>();
			...
			if (subListLength == 0)
			{
				...
				list.set(testIndex, new FastTestList(
					FastConstants.IF_, test.offset, 
					test.lineNumber, elseOffset-test.offset, test, null));
				return;
			}
			
			int beforeSubListOffset = test.offset;
			Instruction beforeElseBlock = subList.get(subListLength-1);
			int afterIfElseOffset = SearchMinusJumpOffset(
				subList, 0, subListLength, -1, beforeElseBlock.offset);
			
			if (afterIfElseOffset != -1)
			{
				if ((subListLength == 1) && 
					(beforeElseBlock.opcode == FastConstants.GOTO))
				{
					CreateBreakAndContinue(
						method, subList, offsetLabelSet, beforeLoopEntryOffset, 
						loopEntryOffset, breakOffset, returnOffset);
	
					ComparisonInstructionAnalyzer.InverseComparison(test);
					list.set(testIndex, new FastTestList(
						FastConstants.IF_, beforeElseBlock.offset, 
						test.lineNumber, elseOffset-beforeElseBlock.offset, 
						test, subList));
					return;
				}
				
				if ((test.branch < 0) &&
					(beforeLoopEntryOffset < afterIfElseOffset) && 
					(afterIfElseOffset <= loopEntryOffset))
				{
					...
					((afterListOffset == -1) || (afterIfElseOffset <= afterListOffset)))
				{
					...
					if ((beforeElseBlock.opcode == FastConstants.GOTO) && 
						(((Goto)beforeElseBlock).GetJumpOffset() == afterIfElseOffset))
					{
						// Remove 'goto'
					...
					length = ExtrackBlock(
						list, subElseList, index, length, afterIfElseOffset);
					...
					if (subElseList.size() > 0)
					{
						...
						list.set(testIndex, new FastTest2Lists(
							FastConstants.IF_ELSE, lastIfElseOffset, 
							test.lineNumber, afterIfElseOffset-lastIfElseOffset, 
							test, subList, subElseList));
						return;
					}
				}
			}
			...
			AnalyzeList(
				classFile, method, subList, localVariables, offsetLabelSet, 
				beforeLoopEntryOffset, loopEntryOffset, beforeSubListOffset, 
				elseOffset, breakOffset, returnOffset);
		...
		{
			list.set(testIndex, new FastTestList(
				FastConstants.IF_, test.offset, test.lineNumber, 
				elseOffset-test.offset, test, null));
		}
	}	
	 ...
	 *                beforeLoopEntryOffset                 afterLoopOffset
	 */
	private static void AnalyzeLookupSwitch(
		ClassFile classFile, Method method, List<Instruction> list, 
		LocalVariables localVariables, IntSet offsetLabelSet, 
		int beforeLoopEntryOffset, int loopEntryOffset, int afterListOffset, 
		int returnOffset, int switchIndex, LookupSwitch ls)
	{
		...
		for (int i=0; i<pairLength; i++)
		{
			...
			{
				pairs[pairIndex++] = new FastSwitch.Pair(
						true, 0, ls.offset + ls.defaultOffset);
				defaultFlag = false;
			}
			
			pairs[pairIndex++] = new FastSwitch.Pair(
					false, ls.keys[i], ls.offset + ls.offsets[i]);
		}
		...
		int switchOpcode = FastConstants.SWITCH;
		
		// SWITCH or SWITCH_ENUM ?
		if (ls.key.opcode == ByteCodeConstants.ARRAYLOAD)
		{
			// switch (1.$SwitchMap$basic$data$TestEnum$enum1[e.ordinal()]) ?
			ArrayLoadInstruction ali = (ArrayLoadInstruction)ls.key;

			if ((ali.arrayref.opcode == ByteCodeConstants.GETSTATIC) && 
				(ali.indexref.opcode == ByteCodeConstants.INVOKEVIRTUAL))
			{
				...
				{
					// SWITCH_ENUM found
					switchOpcode = FastConstants.SWITCH_ENUM;
				}
			}
		}
		...
		AnalyzeSwitch(
			classFile, method, list, localVariables, offsetLabelSet, 
			beforeLoopEntryOffset, loopEntryOffset, afterListOffset, 
			returnOffset, switchIndex, switchOpcode, 
			ls.offset, ls.lineNumber, ls.key, pairs, pairLength);
	}
	 ...
	 *                beforeLoopEntryOffset                 afterLoopOffset
	 */
	private static int AnalyzeTableSwitch(
		ClassFile classFile, Method method, List<Instruction> list, 
		LocalVariables localVariables, IntSet offsetLabelSet, 
		int beforeLoopEntryOffset, int loopEntryOffset, int afterListOffset, 
		int returnOffset, int switchIndex, TableSwitch ts)
	{
		...
		for (int i=0; i<pairLength; i++)
		{
			...
			{
				pairs[pairIndex++] = new FastSwitch.Pair(
						true, 0, ts.offset + ts.defaultOffset);
				defaultFlag = false;
			}
			
			pairs[pairIndex++] = new FastSwitch.Pair(
					false, ts.low + i, ts.offset + ts.offsets[i]);
		}
		...
		if (ts.key.opcode == ByteCodeConstants.ARRAYLOAD)
		{
			// switch($SWITCH_TABLE$basic$data$TestEnum$enum1()[e.ordinal()]) ?
			ArrayLoadInstruction ali = (ArrayLoadInstruction)ts.key;
				...
				if (is.args.size() == 0)
				{
					...
					if (cmr.class_index == classFile.getThisClassIndex())
					{
						...
						if (classFile.getSwitchMaps().containsKey(cnat.name_index))
						{
							...
							if ((iv.objectref.opcode == ByteCodeConstants.ALOAD) &&
								(iv.args.size() == 0))
							{
		...
		if ((classFile.getMajorVersion() >= 51) && 
			(switchOpcode == FastConstants.SWITCH) &&
			(ts.key.opcode == ByteCodeConstants.ILOAD) &&
			(switchIndex > 2))
		{
			if (AnalyzeSwitchString(
					classFile, localVariables, list, switchIndex, ts, pairs))
			{
				// Switch+String found.
					// Remove FastSwitch
		...
		AnalyzeSwitch(
			...
			beforeLoopEntryOffset, loopEntryOffset, afterListOffset, 
			returnOffset, switchIndex, switchOpcode, 
			ts.offset, ts.lineNumber, ts.key, pairs, pairLength);
		
		return switchIndex;
	}
	
	private static boolean AnalyzeSwitchString(
		ClassFile classFile, LocalVariables localVariables, 
		List<Instruction> list, int switchIndex, 
		TableSwitch ts, FastSwitch.Pair[] pairs)
	{
		Instruction instruction = list.get(switchIndex-3);
		if ((instruction.opcode != FastConstants.ASTORE) ||
			(instruction.lineNumber != ts.key.lineNumber))
			return false;
		AStore astore = (AStore)instruction;

		instruction = list.get(switchIndex-2);
		if ((instruction.opcode != FastConstants.ISTORE) ||
			(instruction.lineNumber != astore.lineNumber))
			return false;

		instruction = list.get(switchIndex-1);
		if ((instruction.opcode != FastConstants.SWITCH) ||
			(instruction.lineNumber != astore.lineNumber))
			return false;
		...
		if ((iv.objectref.opcode != FastConstants.ALOAD) ||
			(iv.args.size() != 0))
			return false;
		...
		int tsKeyIloadIndex = ((ILoad)ts.key).index;
		int previousSwitchAloadIndex = ((ALoad)iv.objectref).index;
		HashMap<Integer, Integer> stringIndexes = 
		...
		while (i-- > 0)	
		{
			...
			for (;;)
			{
				...
				if (instruction.opcode == FastConstants.IF_BREAK)
				{
					if (((length == 2) || (length == 3)) &&
						(AnalyzeSwitchStringTestInstructions(
							constants, cmr,
							tsKeyIloadIndex, previousSwitchAloadIndex,
							stringIndexes, 
							((FastInstruction)instruction).instruction, 
							instructions.get(1), FastConstants.CMP_EQ)))
					{
						break;
					}
					else
				...
				else if (instruction.opcode == FastConstants.IF_ELSE)
				{
					...
					if ((ft2l.instructions.size() == 1) &&
						AnalyzeSwitchStringTestInstructions(
							constants, cmr,
							tsKeyIloadIndex, previousSwitchAloadIndex,
							stringIndexes, ft2l.test, 
							ft2l.instructions.get(0), FastConstants.CMP_NE))
					{
		...
		localVariables.removeLocalVariableWithIndexAndOffset(
			tsKeyIloadIndex, ts.key.offset);
			// Remove synthetic local variable string
		localVariables.removeLocalVariableWithIndexAndOffset(
			astore.index, astore.offset);		
			// Replace switch test
		ts.key = astore.valueref;
	
		return true;
	}

	private static boolean AnalyzeSwitchStringTestInstructions(
			ConstantPool constants, ConstantMethodref cmr,
			int tsKeyIloadIndex, int previousSwitchAloadIndex,
			HashMap<Integer, Integer> stringIndexes,
			Instruction test, Instruction value, int cmp)
	{
		...
		if ((ii.cmp != cmp) || 
			(ii.value.opcode != FastConstants.INVOKEVIRTUAL))
			return false;
		
		Invokevirtual ivTest = (Invokevirtual)ii.value;
		
		if ((ivTest.args.size() != 1) ||
			(ivTest.objectref.opcode != FastConstants.ALOAD) ||
			(((ALoad)ivTest.objectref).index != previousSwitchAloadIndex) ||
			(ivTest.args.get(0).opcode != FastConstants.LDC))
			return false;
		
		ConstantMethodref cmrTest = 
			constants.getConstantMethodref(ivTest.index);
		if (cmr.class_index != cmrTest.class_index)
			return false;
		
		ConstantNameAndType cnatTest = 
			constants.getConstantNameAndType(cmrTest.name_and_type_index);
		String descriptorNameTest = 
			constants.getConstantUtf8(cnatTest.descriptor_index);
		if (! descriptorNameTest.equals("(Ljava/lang/Object;)Z"))
			return false;
		
		String methodNameTest = 
			constants.getConstantUtf8(cnatTest.name_index);
		if (! methodNameTest.equals("equals"))
			return false;
		
		stringIndexes.put(
			index,
			Integer.valueOf( ((Ldc)ivTest.args.get(0)).index ));	
	 ...
	 *                |    |   |              |                        |  |
	 *                |    |   beforeListOff. |                        |  |
	 * Offsets        |    loopEntryOffset    switchOffset endLoopOffset  |
	 *                beforeLoopEntryOffset                 afterLoopOffset
	 */
	private static void AnalyzeSwitch(
		...
		LocalVariables localVariables, IntSet offsetLabelSet, 
		int beforeLoopEntryOffset, int loopEntryOffset, int afterListOffset, 
		int returnOffset, int switchIndex, int switchOpcode, int switchOffset, 
		int switchLineNumber, Instruction test, FastSwitch.Pair[] pairs, 
		int pairLength)
	{
		int breakOffset = -1;
		...
		if (index < list.size())
		{
					...
					if (instruction.offset >= afterCaseOffset)
					{
						if (instructions != null)
						{
							...
							if (nbrInstrucrions > 0)
							{
								...
								int breakOffsetTmp = SearchMinusJumpOffset(
									instructions, 0, nbrInstrucrions,
								...
								if (instruction.opcode == FastConstants.GOTO)
								{
									...
									instructions.set(nbrInstrucrions-1, 
										new FastInstruction(
			...
			if (breakOffset != -1)
			{
				int afterSwitchOffset = (breakOffset >= switchOffset) ? 
					breakOffset : list.get(list.size()-1).offset+1;

				// Reduction de 'afterSwitchOffset' via les 'Branch Instructions'	
				Instruction instruction;
				...
				while (i-- > 0)
				{
					...
					if ((instruction.offset <= afterSwitchOffset) ||
						(instruction.offset <= lastSwitchOffset))
						break;
				}
				...
				if (instructions != null)
				{
					...
					if (nbrInstrucrions > 0)
					{
						...
						if (instruction.opcode == FastConstants.GOTO_BREAK)
						{
							...
							AnalyzeList(
								classFile, method, instructions, localVariables, 
								offsetLabelSet, beforeLoopEntryOffset, 
								loopEntryOffset, beforeListOffset, 
								afterListOffset, breakOffset, returnOffset);
							instructions.add(instruction);
						}
						else
		...
		list.set(switchIndex, new FastSwitch(
			switchOpcode, lastSwitchOffset, switchLineNumber, 
			branch, test, pairs));
	}
	
	private static void AddLabels(
		List<Instruction> list, IntSet offsetLabelSet)
	{
	 ...
	 */
	private static boolean SearchInstructionAndAddLabel(
			List<Instruction> list, int labelOffset)
	{
		...
		case FastConstants.INFINITE_LOOP:
			{
				...
				if (fl.instructions != null)
					found = SearchInstructionAndAddLabel(fl.instructions, labelOffset);
			}
			break;	
		case FastConstants.WHILE:
		case FastConstants.DO_WHILE:
		case FastConstants.IF_:
			{
				...
				if ((labelOffset >= ftl.test.offset) && 
		...
		case FastConstants.SYNCHRONIZED:
			{
				FastSynchronized fs = (FastSynchronized)instruction;
				if ((labelOffset >= fs.monitor.offset) && 
		...
		case FastConstants.FOR:
			{
				FastFor ff = (FastFor)instruction;
				if (((ff.init == null) || (labelOffset >= ff.init.offset)) && 
					(ff.instructions != null))
		...
		case FastConstants.IF_ELSE:
			{
				...
				if (labelOffset >= ft2l.test.offset)
					found = 
						SearchInstructionAndAddLabel(ft2l.instructions,  labelOffset) ||
				...
				if (labelOffset >= fs.test.offset)
				{
					...
					if (pairs != null)
						for (int i=pairs.length-1; i>=0 && !found; --i)
						{
							...
							if (instructions != null)
								found = SearchInstructionAndAddLabel(
												instructions, labelOffset);
						}
				}
			}
			break;
		case FastConstants.TRY:
			{
				...
				if (!found && (ft.catches != null))
					...
					{
						found = SearchInstructionAndAddLabel(
								ft.catches.get(i).instructions, labelOffset);
					}
				
				if (!found && (ft.finallyInstructions != null))
					found = SearchInstructionAndAddLabel(
								ft.finallyInstructions, labelOffset);
			}
		}	
		
		if (! found)
			list.set(index, new FastLabel(
				FastConstants.LABEL, labelOffset, 
				instruction.lineNumber, instruction));
