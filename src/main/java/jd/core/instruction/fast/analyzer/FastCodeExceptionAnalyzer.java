...
import jd.core.instruction.fast.visitor.FastCompareInstructionVisitor;
...
{	
			...
			Method method, List<Instruction> list)
	{
		...
		for (int i=fastCodeExceptions.size()-1; i>=1; --i)
		{
			...
			if ((fce1.tryFromOffset == fce2.tryFromOffset) && 
				...
				((fce1.afterOffset == -1) || (fce1.afterOffset > fce2.maxOffset)) &&
				((fce2.afterOffset == -1) || (fce2.afterOffset > fce1.maxOffset)))
			{
				// Append catches
		...
		for (int i=fastCodeExceptions.size()-1; i>=0; --i)
		{
			...
			// Compute afterOffset
			ComputeAfterOffset(
				list, switchCaseOffsets, fastCodeExceptions, fce, i);
			...
			if (array[codeException.handler_pc] == null)
			{
				FastAggregatedCodeException face = 
					new FastAggregatedCodeException(
						codeException.start_pc, codeException.end_pc, 
		...
		List<Instruction> list, FastAggregatedCodeException face)
	{
		...
		case ByteCodeConstants.ASSIGNMENT:
			{
				...
				if (ai.value1.opcode != ByteCodeConstants.ASTORE)
					return false;
		...
		{
			// Aucune instruction 'MonitorExit' n'a ete trouvée. Cas de la 
	...
	{			
		...
		{
			FastCodeExceptionCatch fcec = new FastCodeExceptionCatch(
				fastCodeException.catch_type, fastCodeException.handler_pc);
			fce.catches.add(fcec);		
		}
	...
	private static void ComputeAfterOffset(
			List<Instruction> list, ArrayList<int[]> switchCaseOffsets,
			ArrayList<FastCodeException> fastCodeExceptions,
		...
		case FastConstants.TYPE_118_CATCH_FINALLY:
			{
				...
				while (++index < length)
				{
					...
					{
					case ByteCodeConstants.JSR:
						offsetSet.add(i.offset + ((Jsr)i).value);
						break;
		...
		case FastConstants.TYPE_ECLIPSE_677_CATCH_FINALLY:	
			{
					...
					(list.get(index).opcode == ByteCodeConstants.GOTO))
				{
					...
					{
						// Reduction de 'afterOffset' via les 'Branch Instructions'
						...
						// Reduction de 'afterOffset' via les instructions de
		...
		case FastConstants.TYPE_118_FINALLY:
			{
				...
				while (++index < length)
				{
					...
					if (i.opcode == ByteCodeConstants.JSR)
					{
						int jumpOffset = i.offset + ((Jsr)i).value;
		...
		default:
			{
				...
				// Reduction de 'afterOffset' via les instructions 'switch'
				...
				// Reduction de 'afterOffset' via les instructions de gestion
				...
				// Recherche de la 1ere exception débutant apres 'maxOffset' 
				...
				while (index < length)
				{
					...
					else
					{
						...
						case ByteCodeConstants.XRETURN:
							...
							// saute-t-elle vers l'instuction qui suit ce 
							// 'return' ?
							if (index+1 >= length)
							{
								// Dernier 'return' => 'return' placé apres le 
								// bloc
						...
						case ByteCodeConstants.IFXNULL:
								...
								else 
								{
										...
										(jumpOffsetTmp != afterOffset))
									{
										...
										else
										{
											...
											// 'try-catch-finally' saute vers 
											// cette instuction 
											// => Instruction placée apres le bloc
											fastCodeException.afterOffset = instruction.offset;
										}
									}
									//else
							...
							{
								// Saut negatif
								if ((index > 0) && (instruction.lineNumber != -1))
								{
		...
		FastCodeException fce, FastTry fastTry, int returnOffset)
	{
		...
		case FastConstants.TYPE_CATCH:
			FormatCatch(fce, fastTry);
			break;
		case FastConstants.TYPE_118_FINALLY:
			Format118Finally(fce, fastTry);
			break;
		case FastConstants.TYPE_118_FINALLY_2:
	...
	private static void FormatCatch(FastCodeException fce, FastTry fastTry)
	{
		...
		FormatFastTry_RemoveJsrInstruction(tryInstructions);
		...
		while (i-- > 0)
		{
			...
			if (catchInstructions.size() > 0)
			{
				...
				// Remove JSR instruction in try block before 'return' instruction
				FormatFastTry_RemoveJsrInstruction(catchInstructions);
			}
		}
	}
		...
		FastCodeException fce, FastTry fastTry)
	{
		...
		if (tryInstructions.get(--length).opcode == ByteCodeConstants.JSR)
		{
			...
			jumpOffset = jsr.offset + jsr.value;		
		}
		if (jumpOffset == -1)
			throw new UnexpectedInstructionException();
	...
	private static void Format142(FastCodeException fce, FastTry fastTry)
	{
		List<Instruction> finallyInstructions = fastTry.finallyInstructions;
		...
		Format142_RemoveFinallyInstructionsBeforeReturn(
			visitor, tryInstructions, finallyInstructions);
		...
		if (fastTry.catches != null)
		{
			...
			while (i-- > 0)
			{
				...
				Format142_RemoveFinallyInstructionsBeforeReturn(
					visitor, catchInstructions, finallyInstructions);				
				// Remove first catch instruction in each catch block
				if (catchInstructions.size() > 0)
					catchInstructions.remove(0);
			}		
	...
	private static void Format142_RemoveFinallyInstructionsBeforeReturn(
		FastCompareInstructionVisitor visitor,
		...
		List<Instruction> finallyInstructions)
	{
		...
		int finallyInstructitonsSize = finallyInstructions.size();
		boolean match = (i >= finallyInstructitonsSize) && visitor.visit(
			instructions, finallyInstructions, 
			i-finallyInstructitonsSize, 0, finallyInstructitonsSize);
		...
		while (i-- > 0)
		{
			...
			case ByteCodeConstants.XRETURN:
			case ByteCodeConstants.ATHROW:
				match = (i >= finallyInstructitonsSize) && visitor.visit(
					instructions, finallyInstructions, 
					i-finallyInstructitonsSize, 0, finallyInstructitonsSize);
	...
	private static int FormatFastTry_RemoveJsrInstruction(
		List<Instruction> instructions)
	{
		int jumpOffset = -1;
		int i = instructions.size();
		...
		while (i-- > 1)
		{
			if (instructions.get(i).opcode == ByteCodeConstants.JSR)
			{
				// Remove Jsr instruction
				Jsr jsr = (Jsr)instructions.remove(i);
				jumpOffset = jsr.offset + jsr.value;
			}
		}
		...
		implements Comparable<FastCodeExceptionCatch>
	{
		...
		public FastCodeExceptionCatch(int type, int fromOffset)
		{
			this.type = type;
	...
	public static class FastAggregatedCodeException extends CodeException
	{
		...
		{
			super(start_pc, end_pc, handler_pc, catch_type);
