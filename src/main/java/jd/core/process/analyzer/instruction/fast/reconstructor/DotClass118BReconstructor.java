...
public final class DotClass118BReconstructor
{
        ...
        ReferenceMap referenceMap, ClassFile classFile, List<Instruction> list)
    {
        ...
        while (i-- > 0)
        {
            ...
            String cfrClassName = classFile.getConstantPool().getConstantClassName(cfr.getClassIndex());
            String thisClassName = classFile.getConstantPool().getConstantClassName(classFile.getThisClassIndex());
            if (!cfrClassName.equals(thisClassName) && !thisClassName.startsWith(cfrClassName + '$')) {
                continue;
            }
